...
export async function getUserEnrolledCourses(data,token){
    const toastId = toast.loading("Loading...")
    let result = []
    try {
      console.log(data);
      const response = await apiConnector("POST",GET_USER_ENROLLED_COURSES_API,{data},
...
export async function getInstructorData(data,token) {
    const toastId = toast.loading("Loading...");
    let result = [];
    try{
      console.log(data);
      const response = await apiConnector("POST", GET_INSTRUCTOR_DATA_API,{data}, 
      {
        Authorization: `Bearer ${token}`,
      })
